# NOTE: For clearly debugging stuff in 'run' lines, sometimes echo $output will show you
# errors that bats -x and echo inline in helpers won't.
...
setup_sops() {
    ...
    source "${HELPERS_PATH-$(git rev-parse --show-toplevel)/pkgs/introdus-helpers/helpers.sh}"
}
...
@test "update shared creation rules" {
    ...
    sops_update_age_key users "$user"_"$host" "${AGE_TEST_KEY_3}"
    sops_update_age_key hosts "$host" "${AGE_TEST_KEY_4}"
    sops_add_shared_creation_rules "$user" "$host"

    yq '.creation_rules' "$NIX_SECRETS_DIR"/.sops.yaml >"$TEST_TEMP/creation_rules"
    run grep "$user" "$TEST_TEMP/creation_rules"
    [ "$status" -eq 0 ]

    run grep "$host" "$TEST_TEMP/creation_rules"
    [ "$status" -eq 0 ]
...
@test "add host creation rules to sops" {
    ...
    sops_update_age_key users "$user"_"$host" "${AGE_TEST_KEY_1}"
    sops_update_age_key hosts $host "${AGE_TEST_KEY_2}"
...
@test "add host.yaml file" {
    ...
    run sops_setup_user_age_key "$user" "$host"
    [ "$status" -eq 0 ]
